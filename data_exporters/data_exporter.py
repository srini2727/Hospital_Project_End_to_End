...
@data_exporter
def export_all_tables_sequentially(data, *args, **kwargs):
    """
    Receives data from the upstream block, extracts the list of tables,
    and then loops through the list to process each table one by one.
    """
    # KEY CHANGE: The list of tables is the first element of the input data.
    list_of_tables = data[0]
